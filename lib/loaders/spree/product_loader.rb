    ...
    class ProductLoader < SpreeBaseLoader
      ...
      def add_taxons
        ...
        chain_list.each do |chain|
          ...
          taxons = name_list.map do |name|
            begin
              taxon = @@taxon_klass.where(name: name, parent_id: parent).first
              taxon = parent.children.create(name: name) if taxon.blank?

              puts "Not found or created so now what ?" unless taxon
            rescue => e
              puts e.inspect
              puts e.backtrace.join("\n")
              puts "ERROR : Cannot assign Taxon ['#{name}'] to Product ['#{load_object.name}']"
              next
            end
